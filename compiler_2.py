...
from classes_3 import Generatable, Nonterminal, Token, Terminal, Regexp, Star, Plus, Optional, Sequence, Repeat, Literal_Range
...
print(f'grammar: {len(grammar)}, no_cycle: {len(no_cycle_grammar)}, leftover: {len(leftover_grammar)}')
...
for k,v in grammar.items():
    ...
    v = " | ".join([x.to_string() for x in v])
    print(f'\tG\t{v}')

    if k in no_cycle_grammar.keys():
        v1 = no_cycle_grammar[k]
        v1 = " | ".join([x.to_string() for x in v1])
        print(f'\tG\'\t{v1}')

    if k in leftover_grammar.keys():
        v2 = leftover_grammar[k]
        v2 = " | ".join([x.to_string() for x in v2])
        print(f'\tG\'\'\t{v2}')
