...
def handle_repetition(grammar):
    ...
    for nonterminal, alternatives in grammar.items():
        for alternative in alternatives:
            new_alternative = handle_repeater(new_grammar, nonterminal, alternative)
            new_grammar[nonterminal].append(new_alternative)
    return new_grammar

def handle_repeater(grammar, nonterminal, alternative):
    ...
    for element in alternative.get_arg():
        if isinstance(element, Plus):
            alt = handle_repeater(grammar, nonterminal, element.get_arg())
            alt = alt.get_arg()
            temp = alt
            ...
            alt = handle_repeater(grammar, nonterminal, arg)
            alt = alt.get_arg()
            temp = alt
